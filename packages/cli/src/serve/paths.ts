...
/**
 * Absolute path to the prebuilt React dashboard. The published package carries
 * a copy at `<package>/dashboard` (see `scripts/copy-dashboard.mjs`); inside the
 * monorepo we fall back to the sibling workspace's `packages/dashboard/dist`, so
 * `tsx` dev runs work without the copy step. `src/` and `dist/` are both two
 * levels below the package root, so this resolves the same from either.
 */
export function dashboardDistDir(): string {
  const pkgRoot = path.resolve(__dirname, '..', '..');
  const bundled = path.join(pkgRoot, 'dashboard');
  if (existsSync(path.join(bundled, 'index.html'))) return bundled;
  return path.resolve(pkgRoot, '..', 'dashboard', 'dist');
}
